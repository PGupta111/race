"""Big Red Command Center — FastAPI application entry point."""
import json
import os
import random
import time
...
from database import (
    get_db, init_db, seed_runners,
    get_race_start, set_race_start,
)
from detection import simulate_bib_detection
from timing import LineScanTimer, DepthChecker, _fmt
...
async def startup():
    init_db()
    seed_runners()
    # Restore race clock if it was previously started
    rs = get_race_start()
    if rs:
        timer.start(rs)
...
def list_runners():
    conn = get_db()
    rows = conn.execute("SELECT * FROM runners ORDER BY CAST(bib_number AS INTEGER)").fetchall()
    conn.close()
...
@app.post("/api/checkin/{bib_number}")
async def checkin(
    bib_number: str,
    tshirt: int = Form(0),
...
@app.post("/api/detect")
async def detect_bib(photo: UploadFile = File(...)):
    ...
    result = simulate_bib_detection(str(fpath), known)
...
@app.post("/api/timing/trigger")
async def timing_trigger(
    detected_bib: str   = Form(...),
    depth_mm:     float = Form(800.0),
    photo: Optional[UploadFile] = File(None),
):
    ts       = time.time()
    depth_ok = depth_checker.check(depth_mm)

    photo_path = ""
    if photo and photo.filename:
        fname = f"finish_{detected_bib}_{int(ts*1000)}.jpg"
        (UPLOAD_DIR / fname).write_bytes(await photo.read())
        photo_path = f"/uploads/{fname}"
    ...
    cur = conn.execute(
        """INSERT INTO finish_events
           (bib_number, detected_bib, timestamp, depth_mm, depth_ok, photo_path, status)
           VALUES (?, ?, ?, ?, ?, ?, 'pending')""",
        (detected_bib, detected_bib, ts, depth_mm, 1 if depth_ok else 0, photo_path),
    )
    event_id = cur.lastrowid
    conn.commit()

    event = dict(conn.execute(
        "SELECT * FROM finish_events WHERE id = ?", (event_id,)
    ).fetchone())
    conn.close()
    ...
    return {
        "event_id": event_id,
        "timestamp": ts,
        "formatted": _fmt(ts - timer.race_start) if timer.race_start else "---",
        "depth_ok": depth_ok,
        "depth_mm": depth_mm,
...
# ---------------------------------------------------------------------------
# VAR (Video Assistant Referee) Dashboard
# ---------------------------------------------------------------------------
...
@app.post("/api/var/{event_id}/accept")
...
@app.post("/api/var/{event_id}/override")
async def var_override(
    ...
    conn.execute(
        """UPDATE finish_events
           SET status='overridden', bib_number=?, override_bib=?,
               validated_at=?, notes=?
           WHERE id=?""",
...
@app.post("/api/var/{event_id}/reject")
async def var_reject(
    event_id: int,
    notes:    str = Form(""),
):
    now = datetime.now(timezone.utc).isoformat()
...
@app.post("/api/demo/simulate_finish")
async def demo_simulate():
    """Simulate a random runner crossing the finish line (demo / testing)."""
    conn = get_db()
    runner = conn.execute(
        "SELECT bib_number FROM runners ORDER BY RANDOM() LIMIT 1"
    ).fetchone()
    conn.close()
    if not runner:
    ...
    cur = conn.execute(
        """INSERT INTO finish_events
           (bib_number, detected_bib, timestamp, depth_mm, depth_ok, status)
           VALUES (?, ?, ?, ?, ?, 'pending')""",
        (bib, bib, ts, depth_mm, 1 if depth_ok else 0),
    )
    event_id = cur.lastrowid
    conn.commit()
    event = dict(conn.execute(
